...
void validateCorrectedIsolationFlat(bool forBarrel = true){
  ...
  //histEffRaw->GetYaxis()->SetRangeUser(0.5, 1.0);
  ...
  for(UInt_t ievent = 0; ievent < maxEvents; ievent++){
	...
	continue;
      ...
      // Simple test: use only central barrel
	...
	{ ++ieta; };
      ...
      if(ieta != 0)
	printf("ERROR: ieta %d   eta %f\n", ieta, eleEtaSC->at(iele));
