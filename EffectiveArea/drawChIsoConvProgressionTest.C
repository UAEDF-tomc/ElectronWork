...
void drawChIsoConvProgressionTest(){
  ...
  for(int i=0; i<nRhoBins; i++){
    ...
    TString fnameg = TString::Format("fg_%d",i);
    fg[i] = new TF1(fnameg,"gaus(0)",0,20);
    fg[i]->SetNpx(1000);
    fg[i]->SetLineColor(kRed);
    fg[i]->SetParameters( norm, mean, sigmaGauss);
