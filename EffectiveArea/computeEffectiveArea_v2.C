...
// Cutoffs for isolations are cutoff = A + B*rho
const float cutoff_A[5] = {  2.2816,  2.0476,  2.9455,  2.2719,  2.3464};
const float cutoff_B[5] = {  0.1251,  0.1335,  0.0892,  0.1338,  0.2195};
...
void computeEffectiveArea_v2(EffectiveAreaType eaType = EA_NEUTRAL_TOTAL){
  ...
  for(UInt_t ievent = 0; ievent < maxEvents; ievent++){
	...
	assert(0);
      ...
      float cutoff = cutoff_A[ieta] + rho*cutoff_B[ieta];
      if( iso>0 && iso<cutoff ){ // DEBUG
